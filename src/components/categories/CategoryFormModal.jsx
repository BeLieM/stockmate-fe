        ...
        <Draggable handle=".drag-area" nodeRef={nodeRef}>
          <div ref={nodeRef} className="bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden m-0 text-white">
            ...
            <DialogHeader className="p-5 pb-4 border-b border-zinc-800 bg-zinc-900/50 drag-area cursor-move flex flex-row items-start justify-between select-none">
              ...
              <button type="button" onClick={onClose} className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer mt-0.5">
                <X size={18} />
              </button>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmitSlicing} className="p-5 space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-zinc-500 text-[10px] uppercase font-bold tracking-widest block">Category Name *</label>
                <input 
                  name="name" 
                  value={formData.name} 
                  onChange={handleChange} 
                  className={`w-full bg-zinc-900/50 rounded-lg px-4 py-2.5 text-sm focus:outline-none transition-colors ${errors.name ? 'border-red-500 border focus:border-red-500 placeholder:text-red-500/70' : 'border border-zinc-800 focus:border-[#00E599]'}`} 
                  placeholder={errors.name || "e.g. Minuman, Sembako, Elektronik..."}
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-zinc-500 text-[10px] uppercase font-bold tracking-widest block">Description (Optional)</label>
                <textarea 
                  name="description" 
                  value={formData.description || ''} 
                  onChange={handleChange} 
                  className="w-full bg-zinc-900/50 border border-zinc-800 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#00E599] min-h-[100px] resize-none" 
                  placeholder="Brief description of this category..." 
                />
              </div>

              <div className="flex justify-end gap-3 pt-4 mt-2">
                <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-lg text-sm font-bold bg-zinc-900 text-zinc-400 border border-zinc-800 hover:bg-zinc-800 transition-colors cursor-pointer">
                  Cancel
                </button>
                <button type="submit" className="px-5 py-2.5 rounded-lg text-sm font-bold bg-[#00E599] text-zinc-950 hover:bg-[#00c985] transition-colors cursor-pointer">
